...
class NimblePicker extends React.PureComponent {
  ...
  getEmojiToPreview(categoryIndex, emojiIndex) {
    ...
    const emoji = emojis[emojiIndex];
    const emojiToPreview = getSanitizedData(emoji, this.state.skin, this.props.set, this.props.data);
    var {
      preview
    } = this;
    ...
    const emojiData = this.CUSTOM.filter(customEmoji => customEmoji.id === emojiToPreview.id)[0];
  ...
  handleEmojiKeyDown(e, currentEmoji, {
    category,
    row,
    column
  }) {
    ...
    switch (e.key) {
      ...
      case 'ArrowUp':
        ...
        if (emojiIndex < 0) {
          ...
          let numOfItemsOnLastRow = this.getEmojisInCategory(newCategoryIndex).length % perLine;
